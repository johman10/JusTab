<template>
  <div class="select">
    <label class="select__label" @click="showList">{{ label }}</label>
    <span @click="showList" class="select__bar">
      {{ changeValue }}
    </span>
    <ul class="select__list" v-show="active">
      <li v-for="option in options" :class="['select__list-item ripple', { 'selected': option === changeValue }]" @click="setValue(option)">{{ option }}</li>
    </ul>
  </div>
</template>
