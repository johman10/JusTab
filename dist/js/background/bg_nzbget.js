...
function nghHTML(itemLength) {
  if (serviceData.NG.history.JSON) {
    ...
    historyJson.result.slice(0, itemLength).forEach(function (index, el) {
      historyHTML += '<div class="core-item ng-item-container">' + '<div class="ng-item-name">' + htmlEncode(el.Name) + '</div>' + '<div class="core-item-icon"></div>' + '</div>' + '<div class="ng-collapse core-collapse">' + '<div class="ng-collapse-status">' + htmlEncode(el.Status) + '</div>' + '</div>';
    });
