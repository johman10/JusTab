...
function dnHTML() {
  if (serviceData.DN.JSON) {
    ...
    data.stories.forEach(function (story) {
      ...
      if (story.badge) {
        dnLinks += '<div class="core-item waves-effect dn-link-container dn-link-with-badge">' + '<img src="/img/dn_badges/badge_' + story.badge + '.png" class="dn-badge">';
      } else {
        dnLinks += '<div class="core-item waves-effect dn-link-container">';
      }

      dnLinks += '<a href="' + story.url + '" class="dn-story-url service-link" target="_blank">' + htmlEncode(story.title) + '</a>' + '<a href="https://www.designernews.co/stories/' + story.id + '" class="dn-comments-url" target="_blank">' + htmlEncode(story.comment_count + ' comments - ' + story.vote_count + ' points') + '</a>' + '</div>';
    });
