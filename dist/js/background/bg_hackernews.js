...
function getHackerNewsStories(data, callback) {
  ...
  data.forEach(function (val) {
    ...
    promises.push(ajax('GET', url + apiCall).then(function (data) {
...
function hnHTML() {
  if (serviceData.HN.JSON) {
    ...
    data.forEach(function (story) {
      hn_links += '<div class="core-item waves-effect hn-link-container">';

      if (story.url) {
        hn_links += '<a href="' + story.url + '" class="hn-story-url service-link" target="_blank">';
      } else {
        hn_links += '<a href="https://news.ycombinator.com/item?id=' + story.id + '" class="hn-story-url" target="_blank">';
      }

      hn_links += htmlEncode(story.title) + '</a>' + '<a href="https://news.ycombinator.com/item?id=' + story.id + '" class="hn-comments-url" target="_blank">' + htmlEncode(story.descendants + ' comments - ' + story.score + ' points') + '</a>' + '</div>';
    });
