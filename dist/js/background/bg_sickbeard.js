...
function listSeries(query, HTML, storageName, serviceDataTag) {
  query.forEach(function (episodeData) {
    ...
    HTML += '<div class="sb-item core-item">' + '<div class="sb-poster-container">' + '<img class="sb-poster" src="img/poster_fallback.png" data-echo="' + posterUrl + '">' + '</div>' + '<div class="core-item-content">' + htmlEncode(showname + episodeString) + '</div>' + '<div class="core-item-icon"></div>' + '</div>' + '<div class="sb-collapse core-collapse">' + '<div class="sb-collapse-date">' + htmlEncode(date) + '</div>' + '<div class="sb-collapse-buttons">' + '<div class="icon-button search-icon sb-search-episode waves-effect" data-tvdbid="' + tvdbid + '" data-season="' + season + '" data-episode="' + episode + '"></div>' + '<a class="sb-tvdb-link" href="http://thetvdb.com/?tab=series&id=' + tvdbid + '" target="_blank">' + '<div class="icon-button info-icon sb-tvdb-link-icon waves-effect"></div>' + '</a>' + '<div class="icon-button done-icon sb-mark-episode waves-effect" data-tvdbid="' + tvdbid + '" data-season="' + season + '" data-episode="' + episode + '"></div>' + '</div>' + '</div>';
  });
