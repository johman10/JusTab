  ...
  methods: {
    ...
    sonarrEpisodes() {
      ...
      const endDate = dayjs()
        .add(1, 'months')
        .format('YYYY-MM-DD');
      const params = `apikey=${this.sonarrService.key}&includeSeries=true&start=${startDate}&end=${endDate}`;
      const apiUrl = `${url}/api/v3/calendar?${params}`;
    ...
    sonarrPoster(episode) {
      ...
      const posterUrl = episode.series.images.find(
    ...
    sonarrBuildHeader({ airDate, currentSection }) {
      ...
      if (currentSection !== 'missed' && dateDifference < 0) {
        ...
        return {
          component: this.sonarrSubheader('Soon'),
          currentSection: 'soon',
        };
      } else if (currentSection !== 'later' && dateDifference > 7) {
        return {
          component: this.sonarrSubheader('Later'),
