...
$('#couchpotato').on('click', '#couchpotato .cp_item', function(event) {
  var collapseItem = $(this).next('#couchpotato .cp_collapse');
  var collapseIcon = $(this).find('#couchpotato .cp_collapse_icon');
  if (collapseItem.attr('opened') == 'false') {
    $('#couchpotato .cp_collapse').attr('opened', false);
    $('#couchpotato .cp_item').css('background-color', '#fafafa');
    $('#couchpotato .cp_collapse_icon_container').css('background-color', '#fafafa');
    $('#couchpotato .cp_collapse_icon[icon=expand-less]').fadeOut(165, function() {
